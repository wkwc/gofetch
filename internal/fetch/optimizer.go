...
func scaleWorkers(totalSize int64, cores int) int {
	...
	w := int(chunkCount) / 8
	// Small-file floor is ABSOLUTE (4), not cores-relative: optimal
	// concurrency for network-bound work doesn't scale with CPU count.
	// Measured on a shaped ~35 Mbps link (10 MB): 4 workers 2.7 s vs
	// 8 workers 4.1 s — over-parallelizing past ~4 connections halves
	// aggregate throughput on shaped paths, and the old max(4, cores/2)
	// floor picked exactly that worst point on many-core boxes.
	if w < 4 {
		return 4
	}
	if w > 32 {
